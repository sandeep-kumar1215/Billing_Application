...
class Bill_App():
    def __init__(self,root):
       ...
       #=================== Cosmetic Frame ===============
        ...
        g1_lbl = Label(F4, text="Milk", font=("times new roman", 16, "bold"), bg=bg_color,
                       fg="lightgreen").grid(
            row=0, column=0, padx=10, pady=10, sticky="w")
        g1_txt = Entry(F4, width=10,textvariable=self.milk, font=("times new roman", 16, "bold"), bd=5, relief=SUNKEN).grid(row=0,
                                                                                                     column=1,
                                                                                                     padx=10,
                                                                                                     pady=10)
        ...
        Gen_btn = Button(btn_F, text="Generate Bill",command=self.generate_bill, bg="cadetblue", fg="white", bd=3, pady=11, width=11,
    ...
    def total(self):
            ...
            self.b_mk_p = self.milk.get() * 50
            self.b_s_p = self.sprite.get() * 70
            self.b_l_p = self.limca.get() * 75
            self.b_f_p =  self.frooti.get() * 40
            self.b_m_p = self.maza.get() * 60
            self.b_c_p = self.cock.get() * 30
    ...
    def generate_bill(self):
     ...
     try:
        ...
        if phone_number_int < 1000000000 or phone_number_int > 9999999999:
            messagebox.showerror("Error", "Enter a valid phone number within the range (1000000000 - 9999999999)")
            return   
     except ValueError:   
        messagebox.showerror("Error", "Enter a valid phone number (digits only)")
        return   
    ...
    def bill_area(self):
        ...
        if self.c_name.get()=="":
            ...
            # Beverages ============
            if self.milk.get() != 0:
                self.txtarea.insert(END, f"\n Milk \t\t{self.milk.get()}\t{self.b_mk_p}")

            if self.sprite.get() != 0:
                self.txtarea.insert(END, f"\n Sprite\t\t{self.sprite.get()}\t{self.b_s_p}")

            if self.limca.get() != 0:
                self.txtarea.insert(END, f"\n Limca  \t\t{self.limca.get()}\t{self.b_l_p}")

            if self.frooti.get() != 0:
                self.txtarea.insert(END, f"\n Frooti\t\t{self.frooti.get()}\t{self.b_f_p}")

            if self.maza.get() != 0:
                self.txtarea.insert(END, f"\n Maza\t\t{self.maza.get()}\t{self.b_m_p}")

            if self.cock.get() != 0:
                self.txtarea.insert(END, f"\n Cock \t\t{self.cock.get()}\t{self.b_c_p}")
